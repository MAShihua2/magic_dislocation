...
functions for deleting and moving atoms.

  Typical usage example:

  pipeline = DeleteMovePipeline(config)
  pipeline.run()
"""
...
from magic_dislocation.operations.delete import LayerBasedAtomDeleter
...
class DeleteDisplacePipeline(BasePipeline):
    """
    ...
    def init_submodules(self):
        ...
        self.atom_deleter = LayerBasedAtomDeleter(self.config)
    ...
    def delete_atoms(self):
        ...
        delete atoms.
        """
        results = self.atom_deleter.run()
        
        if self.config["visualize_delete"]:
            vis_input = {
                "deleted_atoms_positions": results["selected_atoms_positions"],
                "other_atoms_positions": results["unselected_atoms_positions"]
            }
            self.visualizer.show(vis_input)
            
        self.config["deleted_atoms_indexes"] = results["selected_atoms_indexes"]
    ...
    def run(self):
        ...
        self.delete_atoms()
        self.logger.info("Delete atoms for S plane successfully!")
        self.select_atoms()
