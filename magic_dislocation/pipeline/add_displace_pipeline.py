...
functions for adding and moving atoms.
...
from magic_dislocation.operations.add import LayerBasedAtomAdder
...
class AddDisplacePipeline(BasePipeline):
    """
    A class for adding and moving atoms.
    """
    ...
    def init_submodules(self):
        ...
        self.atom_adder = LayerBasedAtomAdder(self.config)
    ...
    def add_atoms(self):
        ...
        Add atoms.
        """
        results = self.atom_adder.run()
        
        if self.config["visualize_add"]:
            vis_input = {"added_atoms_positions": results["added_atoms_positions"],
                         "original_atoms_positions": self.config["positions"]}
            self.visualizer.show(vis_input)
            
        self.config["added_atoms_indexes"] = results["added_atoms_indexes"]
        self.config["added_atoms_positions"] = results["added_atoms_positions"]
        self.config["added_atoms_types"] = results["added_atoms_types"]
    ...
    def move_atoms(self,):
        ...
        # print("min z and max z of target atoms: ", min(vis_input["target_atoms_positions"][:, 2]), max(vis_input["target_atoms_positions"][:, 2]))
        ...
        min_z = min(self.config["moved_atoms_positions"][:, 2])
        max_z = max(self.config["moved_atoms_positions"][:, 2])
        self.logger.info("min z and max z of moved atoms: {}, {}".format(min_z, max_z))
    ...
    def run(self):
        ...
        self.add_atoms()
        self.logger.info("Add new atoms for S plane successfully!")
        self.select_atoms()
        self.logger.info("Select atoms to be moved successfully!")
        self.move_atoms()
